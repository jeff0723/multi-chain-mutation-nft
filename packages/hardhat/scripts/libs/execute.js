...
async function executeAptosExample(chains, args, wallet, example) {
  ...
  evmChain.gateway = new Contract(
    evmChain.gateway,
    AxelarGatewayContract.abi,
    connectedWallet
  );
  evmChain.gasService = new Contract(
    evmChain.gasService,
...
function getSourceChain(chains, args, exampleSourceChain) {
  return chains.find(
    (chain) => chain.name === (args[0] || exampleSourceChain || "Avalanche")
  );
}
...
function getDestChain(chains, args, exampleDestinationChain) {
  return chains.find(
    (chain) => chain.name === (args[1] || exampleDestinationChain || "Fantom")
  );
}
...
function listenForGMPEvent(env, source, startBlockNumber) {
  ...
  const callContractFilter = gateway.filters.ContractCall(
    source.contract.address
  );
  const callContractWithTokenFilter = gateway.filters.ContractCallWithToken(
    source.contract.address
  );
  ...
  const eventHandler = (...args) => {
    ...
    if (env === "testnet") {
      console.log(
        `You can track the GMP transaction status on https://testnet.axelarscan.io/gmp/${event.transactionHash}\n`
      );
    }
  };
