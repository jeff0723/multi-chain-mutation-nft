...
const MintPage = (props: Props) => {
  ...
  const renderButtonText = () => {
    ...
    if (status == Status.complete)
      return <div className="text-cyan-400">Success</div>;
  };
  return (
    <div>
      <Header />
      ...
      <div className="p-8">
        ...
        <button
          disabled={
            (status == Status.noAction && !previewURL) ||
            (status == Status.uploading && !isValidImageUrl()) ||
            (status != Status.complete && status != Status.noAction)
          }
          onClick={handleGenerateImage}
          className="mt-4 w-full p-4 rounded-2xl bg-white text-black font-bold hover:bg-opacity-50 disabled:bg-opacity-60"
        >
          {renderButtonText()}
        </button>
